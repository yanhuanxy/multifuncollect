...
public class OssUtils {
    ...
    public static File getLocalSaveFile(String fileUrl) {
        String localSavePath = OssUtils.getStaticPath() + fileUrl;
        return new File(localSavePath);
    }

    public static File getCacheFile(String fileUrl) {
        String cachePath = OssUtils.getStaticPath() + "cache" + File.separator + fileUrl;

        return new File(cachePath);
    }

    public static File getTempFile(String fileUrl) {
        String tempPath = OssUtils.getStaticPath() + "temp" + File.separator + fileUrl;
        File tempFile = new File(tempPath);
        File parentFile = tempFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new OssException("创建temp目录失败：目录路径："+ parentFile.getPath());
            }
        }
    ...
    public static File getProcessFile(String fileUrl) {
        String processPath = OssUtils.getStaticPath() + "temp" + File.separator + "process" + File.separator + fileUrl;
        File processFile = new File(processPath);
        File parentFile = processFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new OssException("创建process目录失败：目录路径："+ parentFile.getPath());
            }
        }
        return processFile;
    }
    ...
    public static String urlDecode(String url){
        ...
        try {
            ...
            throw new OssException("不支持的编码格式", e);
        }
        return  decodeUrl;
    }
     ...
     * @return 返回上传文件路径
     */
    public static String getUploadFileUrl(String userCode, String identifier, String extendName) {

        SimpleDateFormat formater = new SimpleDateFormat("yyyyMMdd");
        String path = Objects.isNull(userCode)? ROOT_PATH : userCode + "/" + formater.format(new Date()) + "/";

        File dir = new File(OssUtils.getStaticPath() + path);

        if (!dir.exists()) {

            boolean result = dir.mkdirs();
            if (!result) {
                throw new OssException("创建upload目录失败：目录路径："+ dir.getPath());
            }
    ...
    public static String getObjectNameByFileUrl(String fileUrl) {
    ...
    public static String formatPath(String path) {
        path = OssUtils.pathSplitFormat(path);
        if ("/".equals(path)) {
            return path;
        }
        if (path.endsWith("/")) {
