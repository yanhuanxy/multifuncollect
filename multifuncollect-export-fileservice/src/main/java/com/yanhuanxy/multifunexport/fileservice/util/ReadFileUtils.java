...
//            pdDocument = pdfParser.getPDDocument();
    ...
    private static String readTxt(InputStream inputStream, String extendName) throws IOException {
        try {
            ...
            return org.apache.commons.io.IOUtils.toString(result, "UTF-8");
        } finally {
            IOUtils.closeQuietly(inputStream);
        }
    }
    ...
    public static String readWord(InputStream inputStream, String fileType) {
        ...
        try {
            if ("doc".equalsIgnoreCase(fileType)) {
                ...
                logger.warn("此文件不是word文件！");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("Word 读取异常",e);
        } finally {
            IOUtils.closeQuietly(inputStream);
        }
...
//                buffer = extractor.getText();
//                extractor.close();
                throw new OssException("抱歉 pptx 文件暂不支持！");
            }
        } catch (IOException e) {
            logger.error("ppt文件读取异常",e);
        }
        return buffer;
    }
}